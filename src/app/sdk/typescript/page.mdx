...
```typescript
import * as marlo from '@marlo/sdk';
...
if (learnings) {
  const learningsText = learnings.learnings_text;
  if (learningsText) {
...
**Returns:** A `LearningState` object, or `null` if unavailable.
...
- `learnings_text` (string): Formatted text of all active learnings, ready to inject into prompts.
- `active` (array): Individual learning objects with details.
...
async function handleMessage(userInput: string, threadId: string): Promise<string> {
  ...
  if (learnings) {
    const learningsText = learnings.learnings_text as string | undefined;
    if (learningsText) {
      systemPrompt += `\n\nLearnings:\n${learningsText}`;
    }
  }
